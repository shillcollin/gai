...
// Client implements the core.Provider interface for Groq's chat completions API.
type Client struct {
...
func (c *Client) GenerateText(ctx context.Context, req core.Request) (_ *core.TextResult, err error) {
	ctx, recorder := obs.StartRequest(ctx, "providers.groq.GenerateText",
...
func (c *Client) StreamText(ctx context.Context, req core.Request) (*core.Stream, error) {
	ctx, recorder := obs.StartRequest(ctx, "providers.groq.StreamText",
		attribute.String("ai.provider", "groq"),
...
func (c *Client) GenerateObject(ctx context.Context, req core.Request) (_ *core.ObjectResultRaw, err error) {
	ctx, recorder := obs.StartRequest(ctx, "providers.groq.GenerateObject",
		attribute.String("ai.provider", "groq"),
...
func (c *Client) StreamObject(ctx context.Context, req core.Request) (*core.ObjectStreamRaw, error) {
	ctx, recorder := obs.StartRequest(ctx, "providers.groq.StreamObject",
		attribute.String("ai.provider", "groq"),
...
func (c *Client) doRequest(ctx context.Context, method, path string, payload any) (io.ReadCloser, error) {
	...
	if resp.StatusCode >= 400 {
		...
		return nil, fmt.Errorf("groq: %s: %s", resp.Status, data)
	}
...
func (c *Client) consumeStream(ctx context.Context, body io.ReadCloser, stream *core.Stream) {
	...
	for scanner.Scan() {
		...
		if data == "[DONE]" {
			...
			stream.Push(core.StreamEvent{Type: core.EventFinish, Timestamp: time.Now(), Schema: "gai.events.v1", Seq: seq, StreamID: "stream", Model: finalModel, Provider: "groq", Usage: lastUsage})
			stream.SetMeta(core.StreamMeta{Model: finalModel, Provider: "groq", Usage: lastUsage})
			return
		}
		seq++
		...
		if text := choice.Delta.JoinText(); text != "" {
			stream.Push(core.StreamEvent{Type: core.EventTextDelta, TextDelta: text, Timestamp: time.Now(), Schema: "gai.events.v1", Seq: seq, StreamID: delta.ID, Model: delta.Model, Provider: "groq"})
		}
		...
		if choice.FinishReason != "" {
			stream.Push(core.StreamEvent{Type: core.EventFinish, FinishReason: &core.StopReason{Type: choice.FinishReason}, Timestamp: time.Now(), Schema: "gai.events.v1", Seq: seq, StreamID: delta.ID, Model: delta.Model, Provider: "groq", Usage: lastUsage})
			stream.SetMeta(core.StreamMeta{Model: delta.Model, Provider: "groq", Usage: lastUsage})
		}
	}
