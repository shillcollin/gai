...
func TestIntegration_CreateMessage_SimpleText(t *testing.T) {
	...
	client := vango.NewClient(vango.WithProviderKey("anthropic", apiKey))
...
func TestIntegration_Stream_SimpleText(t *testing.T) {
	...
	stream, err := client.Messages.Stream(ctx, &vango.MessageRequest{
		Model: "anthropic/claude-sonnet-4",
		Messages: []vango.Message{
	...
	resp, err := client.Messages.Create(ctx, &vango.MessageRequest{
		...
		Messages: []vango.Message{
			{Role: "user", Content: []vango.ContentBlock{
				vango.Text("What color is this image? Reply with just the color name."),
				// Use a tiny image URL that Anthropic can fetch
				vango.ImageURL("https://upload.wikimedia.org/wikipedia/commons/thumb/e/e8/Red_square.svg/50px-Red_square.svg.png"),
			}},
		},
		MaxTokens: 50,
	})
