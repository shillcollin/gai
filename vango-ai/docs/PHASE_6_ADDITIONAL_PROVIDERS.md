...
import (
    ...
    "github.com/vango-ai/vango/pkg/core/providers/gemini"
        ...
        t.Run(p.name, func(t *testing.T) {
            ...
            resp, err := testClient.Messages.Create(testCtx, &vango.MessageRequest{
                ...
                Messages: []vango.Message{
                    {Role: "user", Content: vango.Text("Say 'Hello' and nothing else.")},
                },
                MaxTokens: 10,
            })
    ...
    for _, p := range testProviders {
        t.Run(p.name, func(t *testing.T) {
            ...
            stream, err := testClient.Messages.Stream(testCtx, &vango.MessageRequest{
                Model: p.model,
                Messages: []vango.Message{
                    {Role: "user", Content: vango.Text("Count from 1 to 3.")},
                },
                MaxTokens: 50,
            })
            require.NoError(t, err)
    ...
    for _, p := range testProviders {
        t.Run(p.name, func(t *testing.T) {
            ...
            resp, err := testClient.Messages.Create(testCtx, &vango.MessageRequest{
                Model: p.model,
                Messages: []vango.Message{
                    {Role: "user", Content: vango.Text("Get the weather in Paris")},
                },
                Tools: []vango.Tool{{
                    Type:        "function",
                    Name:        "get_weather",
