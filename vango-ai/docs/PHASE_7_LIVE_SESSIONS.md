...
func TestGracePeriod_UserContinues(t *testing.T) {
    ...
    stream, err := client.Messages.RunStream(ctx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4-20250514",
    }, vango.WithLive(), vango.WithGracePeriod(vango.GracePeriodConfig{
        Enabled:    true,
        DurationMs: 5000,
    }))
    ...
    for event := range stream.Events() {
        if e, ok := event.(*vango.InputCommittedEvent); ok {
...
func TestGracePeriod_Expires(t *testing.T) {
    ...
    for {
        select {
        case event := <-stream.Events():
            if _, ok := event.(*vango.GracePeriodExpiredEvent); ok {
                gotExpired = true
            }
            if _, ok := event.(*vango.AudioDeltaEvent); ok && gotExpired {
