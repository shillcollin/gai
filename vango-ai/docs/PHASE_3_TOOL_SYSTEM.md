...
// Example:
//
//     tool := vango.FuncAsTool("get_weather", "Get weather for a location",
...
func TestRun_WebSearch(t *testing.T) {
    ...
    result, err := client.Messages.Run(context.Background(),
        &vango.MessageRequest{
            Model: "anthropic/claude-sonnet-4",
            Messages: []vango.Message{
                {Role: "user", Content: vango.Text("What is the current stock price of Apple? Use web search.")},
            },
...
func TestRun_MaxToolCalls(t *testing.T) {
    ...
    result, err := client.Messages.Run(context.Background(),
        &vango.MessageRequest{
            Model: "anthropic/claude-sonnet-4",
            Messages: []vango.Message{
                {Role: "user", Content: vango.Text("Search for 10 different things.")},
            },
            Tools: []vango.Tool{vango.WebSearch()},
        },
...
func TestRunStream_WithHooks(t *testing.T) {
    ...
    stream, err := client.Messages.RunStream(context.Background(),
        &vango.MessageRequest{
            Model: "anthropic/claude-sonnet-4",
            Messages: []vango.Message{
                {Role: "user", Content: vango.Text("What is 2+2? Search the web to confirm.")},
            },
            Tools: []vango.Tool{vango.WebSearch()},
        },
    ...
    for event := range stream.Events() {
        if _, ok := event.(vango.ToolCallStartEvent); ok {
