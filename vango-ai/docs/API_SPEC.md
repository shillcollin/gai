...
The "brain" of Vango AI. This Go library contains all the complex logic:
...
client := Vango AI.NewClient() // Reads ANTHROPIC_API_KEY, OPENAI_API_KEY from env

resp, err := client.Messages.Create(ctx, &Vango AI.MessageRequest{
    Model: "anthropic/claude-sonnet-4",
    Messages: []Vango AI.Message{
        {Role: "user", Content: Vango AI.Text("Hello!")},
    },
})
```
...
5. SDK returns `*Vango AI.Response`
...
client := Vango AI.NewClient(
    Vango AI.WithBaseURL("http://Vango AI-proxy.internal:8080"),
    Vango AI.WithAPIKey("Vango AI_sk_..."),
)
...
client = Vango AI.Client(
    base_url="https://api.Vango AI.dev",
...
```
[Trace: req_abc123]
├─ Vango AI.handle_request (2341ms)
│  ├─ Vango AI.auth.validate (2ms)
│  ├─ Vango AI.translate.request (5ms)
│  ├─ Vango AI.provider.anthropic (2300ms)
│  │  └─ http.request api.anthropic.com (2290ms)
│  ├─ Vango AI.translate.response (3ms)
│  └─ Vango AI.response.encode (1ms)
```
...
- **Hosted**: `https://api.Vango AI.dev/openapi.yaml`
