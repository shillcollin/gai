...
import (
    ...
    "github.com/vango-ai/vango"
)

var (
    testClient *vango.Client
...
func TestMain(m *testing.M) {
    // Setup
    testClient = vango.NewClient(
        vango.WithTimeout(60*time.Second),
        vango.WithRetries(2),
    )
...
func TestMessages_Create_SimpleText(t *testing.T) {
    ...
    resp, err := testClient.Messages.Create(testCtx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
            {Role: "user", Content: vango.Text("Say exactly: 'Hello, World!' and nothing else.")},
        },
        MaxTokens: 50,
    })
    ...
    assert.Equal(t, vango.StopReasonEndTurn, resp.StopReason)
}
...
func TestMessages_Create_MultiTurn(t *testing.T) {
    ...
    // First turn
    resp1, err := testClient.Messages.Create(testCtx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
            {Role: "user", Content: vango.Text("My name is Alice. Remember this.")},
        },
        MaxTokens: 50,
    })
    require.NoError(t, err)
    ...
    resp2, err := testClient.Messages.Create(testCtx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
            {Role: "user", Content: vango.Text("My name is Alice. Remember this.")},
            {Role: "assistant", Content: resp1.Content},
            {Role: "user", Content: vango.Text("What is my name?")},
        },
        MaxTokens: 50,
    })
    require.NoError(t, err)
    ...
    resp, err := testClient.Messages.Create(testCtx, &vango.MessageRequest{
        ...
        Messages: []vango.Message{
            {Role: "user", Content: []vango.ContentBlock{
                vango.Text("What animal is in this image? Reply with just the animal name."),
...
func TestMessages_Create_VisionURL(t *testing.T) {
    ...
    resp, err := testClient.Messages.Create(testCtx, &vango.MessageRequest{
        ...
        Messages: []vango.Message{
            {Role: "user", Content: []vango.ContentBlock{
                vango.Text("What is shown in this image? Be brief."),
                vango.ImageURL("https://upload.wikimedia.org/wikipedia/commons/thumb/3/3a/Cat03.jpg/1200px-Cat03.jpg"),
            }},
        },
        MaxTokens: 50,
    })
...
func TestMessages_Create_Temperature(t *testing.T) {
    ...
    resp, err := testClient.Messages.Create(testCtx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
            {Role: "user", Content: vango.Text("What is 2+2? Reply with just the number.")},
        },
        Temperature: &temp,
        MaxTokens:   10,
    })
...
func TestMessages_Create_MaxTokens(t *testing.T) {
    ...
    resp, err := testClient.Messages.Create(testCtx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
            {Role: "user", Content: vango.Text("Write a very long essay about the history of computers.")},
        },
        MaxTokens: 10, // Very short
    })

    require.NoError(t, err)
    assert.Equal(t, vango.StopReasonMaxTokens, resp.StopReason)
...
func TestMessages_Create_StopSequences(t *testing.T) {
    ...
    resp, err := testClient.Messages.Create(testCtx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
            {Role: "user", Content: vango.Text("Count from 1 to 10, one number per line.")},
        },
    ...
    require.NoError(t, err)
    assert.Equal(t, vango.StopReasonStopSequence, resp.StopReason)
...
func TestMessages_Create_StructuredOutput(t *testing.T) {
    ...
    resp, err := testClient.Messages.Create(testCtx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
            {Role: "user", Content: vango.Text("Extract: John Smith is 30 years old and works at Acme Corp.")},
        },
        OutputFormat: &vango.OutputFormat{
            Type: "json_schema",
            JSONSchema: &vango.JSONSchema{
                Type: "object",
...
func TestMessages_Extract(t *testing.T) {
    ...
    var person Person
    _, err := testClient.Messages.Extract(testCtx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
            {Role: "user", Content: vango.Text("Extract: Jane Doe, 25, works at TechCo")},
        },
    }, &person)
...
func TestMessages_Stream_SimpleText(t *testing.T) {
    ...
    stream, err := testClient.Messages.Stream(testCtx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
...
func TestMessages_Stream_Cancel(t *testing.T) {
    ...
    stream, err := testClient.Messages.Stream(testCtx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
            {Role: "user", Content: vango.Text("Write a very long story.")},
        },
        MaxTokens: 2000,
    })
    require.NoError(t, err)
...
func TestMessages_Create_WithToolDefinition(t *testing.T) {
    ...
    require.NoError(t, err)
    assert.Equal(t, vango.StopReasonToolUse, resp.StopReason)
...
func TestMessages_Create_WebSearch(t *testing.T) {
    ...
    resp, err := testClient.Messages.Create(testCtx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
            {Role: "user", Content: vango.Text("What was the top news story today? Use web search.")},
        },
    ...
    if resp.StopReason == vango.StopReasonToolUse {
        toolUses := resp.ToolUses()
...
func TestMessages_Create_ToolResult(t *testing.T) {
    ...
    resp1, err := testClient.Messages.Create(testCtx, &vango.MessageRequest{
        ...
        ToolChoice: &vango.ToolChoice{Type: "tool", Name: "get_weather"},
        MaxTokens:  200,
    })
    require.NoError(t, err)
    require.Equal(t, vango.StopReasonToolUse, resp1.StopReason)
    ...
    resp2, err := testClient.Messages.Create(testCtx, &vango.MessageRequest{
        ...
        Messages: []vango.Message{
            ...
            {Role: "assistant", Content: resp1.Content},
            {Role: "user", Content: []vango.ContentBlock{
                vango.ToolResult(toolUses[0].ID, []vango.ContentBlock{
                    vango.Text("Weather in Tokyo: 22°C, sunny"),
                }),
            }},
        },
        Tools: []vango.Tool{
            {
                ...
                Name:        "get_weather",
                Description: "Get weather",
                InputSchema: &vango.JSONSchema{Type: "object"},
            },
        },
        MaxTokens: 200,
    })
    ...
    require.NoError(t, err)
    assert.Equal(t, vango.StopReasonEndTurn, resp2.StopReason)
    assert.Contains(t, resp2.TextContent(), "22")
}

func TestFuncAsTool_SchemaGeneration(t *testing.T) {
    tool := vango.FuncAsTool("test_tool", "A test tool",
...
func TestMessages_Run_BasicToolExecution(t *testing.T) {
    ...
    weatherTool := vango.FuncAsTool("get_weather", "Get weather for a location",
        func(ctx context.Context, input struct {
    ...
    result, err := testClient.Messages.Run(testCtx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
            {Role: "user", Content: vango.Text("What's the weather in San Francisco?")},
        },
    ...
    assert.Equal(t, vango.RunStopEndTurn, result.StopReason)
...
func TestMessages_Run_MultipleToolCalls(t *testing.T) {
    ...
    weatherTool := vango.FuncAsTool("get_weather", "Get weather",
        func(ctx context.Context, input struct {
            Location string `json:"location"`
        }) (string, error) {
            mu.Lock()
...
func TestMessages_Run_MaxToolCallsLimit(t *testing.T) {
    ...
    infiniteTool := vango.FuncAsTool("do_something", "Do something that requires another call",
    ...
    result, err := testClient.Messages.Run(testCtx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
            {Role: "user", Content: vango.Text("Keep calling do_something until I tell you to stop.")},
        },
        Tools:      []vango.Tool{infiniteTool.Tool},
        ToolChoice: &vango.ToolChoice{Type: "tool", Name: "do_something"},
    },
    ...
    assert.Equal(t, vango.RunStopMaxToolCalls, result.StopReason)
}

func TestMessages_Run_MaxTurnsLimit(t *testing.T) {
    requireAnthropicKey(t)

    tool := vango.FuncAsTool("think", "Think about something",
        func(ctx context.Context, input struct{}) (string, error) {
    ...
    result, err := testClient.Messages.Run(testCtx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
            {Role: "user", Content: vango.Text("Think about many things, one at a time.")},
        },
        Tools:      []vango.Tool{tool.Tool},
        ToolChoice: &vango.ToolChoice{Type: "any"},
    },
    ...
    assert.Equal(t, vango.RunStopMaxTurns, result.StopReason)
}
...
func TestMessages_Run_CustomStopCondition(t *testing.T) {
    ...
    result, err := testClient.Messages.Run(testCtx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
            {Role: "user", Content: vango.Text("Count from 1 to 100. At some point say DONE.")},
        },
        MaxTokens: 500,
    },
        vango.WithStopWhen(func(resp *vango.MessageResponse) bool {
            return strings.Contains(resp.TextContent(), "DONE")
        }),
        vango.WithMaxTurns(10),
    )

    require.NoError(t, err)
    // Either stopped by custom condition or naturally
    assert.True(t,
        result.StopReason == vango.RunStopCustom ||
            result.StopReason == vango.RunStopEndTurn)
}

func TestMessages_Run_Timeout(t *testing.T) {
    requireAnthropicKey(t)

    _, err := testClient.Messages.Run(testCtx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
            {Role: "user", Content: vango.Text("Write a very long essay.")},
        },
        MaxTokens: 4000,
    },
        vango.WithTimeout(1*time.Millisecond), // Very short timeout
    )
...
func TestMessages_Run_Hooks(t *testing.T) {
    ...
    tool := vango.FuncAsTool("greet", "Say hello",
    ...
    _, err := testClient.Messages.Run(testCtx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
            {Role: "user", Content: vango.Text("Greet Alice")},
        },
        Tools: []vango.Tool{tool.Tool},
    },
        vango.WithTools(tool),
        ...
        vango.WithAfterResponse(func(resp *vango.MessageResponse) {
            mu.Lock()
            afterResponseCount++
            mu.Unlock()
        }),
...
func TestMessages_Run_UsageAggregation(t *testing.T) {
    ...
    tool := vango.FuncAsTool("calc", "Calculate",
    ...
    result, err := testClient.Messages.Run(testCtx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
            {Role: "user", Content: vango.Text("Calculate 1+1, then 2+2")},
        },
        Tools: []vango.Tool{tool.Tool},
    },
        vango.WithTools(tool),
...
func TestMessages_Stream_VoiceOutput(t *testing.T) {
    ...
    stream, err := testClient.Messages.Stream(testCtx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
            {Role: "user", Content: vango.Text("Say a greeting in one sentence.")},
        },
        Voice: &vango.VoiceConfig{
            Output: &vango.VoiceOutputConfig{
                Provider: "elevenlabs",
    ...
    for event := range stream.Events() {
        if ad, ok := event.(vango.AudioDeltaEvent); ok {
            gotAudioDelta = true
...
func TestAudio_Transcribe_Deepgram(t *testing.T) {
    ...
    transcript, err := testClient.Audio.Transcribe(testCtx, &vango.TranscribeRequest{
        Audio:    audioData,
        Provider: "deepgram",
        Model:    "nova-2",
    })
...
func TestAudio_Synthesize_ElevenLabs(t *testing.T) {
    ...
    result, err := testClient.Audio.Synthesize(testCtx, &vango.SynthesizeRequest{
...
func TestAudio_Synthesize_WithSpeed(t *testing.T) {
    ...
    normalResult, err := testClient.Audio.Synthesize(testCtx, &vango.SynthesizeRequest{
    ...
    fastResult, err := testClient.Audio.Synthesize(testCtx, &vango.SynthesizeRequest{
...
func TestAudio_StreamSynthesize(t *testing.T) {
    ...
    stream, err := testClient.Audio.StreamSynthesize(testCtx, &vango.SynthesizeRequest{
...
func TestError_InvalidModel(t *testing.T) {
    ...
    _, err := testClient.Messages.Create(testCtx, &vango.MessageRequest{
    ...
    assert.Equal(t, vango.ErrNotFound, apiErr.Type)
}
...
func TestError_InvalidRequest(t *testing.T) {
    ...
    var apiErr *vango.Error
    require.True(t, errors.As(err, &apiErr))
    assert.Equal(t, vango.ErrInvalidRequest, apiErr.Type)
}

func TestError_AuthenticationError(t *testing.T) {
    // Create client with invalid key
    badClient := vango.NewClient(
        vango.WithProviderKey("anthropic", "invalid-key"),
    )

    _, err := badClient.Messages.Create(testCtx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
            {Role: "user", Content: vango.Text("Hello")},
        },
    ...
    var apiErr *vango.Error
    require.True(t, errors.As(err, &apiErr))
    assert.Equal(t, vango.ErrAuthentication, apiErr.Type)
}
...
func TestError_ProviderNotConfigured(t *testing.T) {
    ...
    _, err := emptyClient.Messages.Create(testCtx, &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
            {Role: "user", Content: vango.Text("Hello")},
        },
