...
resp, err := client.Messages.Create(ctx, &vango.MessageRequest{
    ...
    Messages: []vango.Message{
        {Role: "user", Content: vango.Text("Hello, how are you?")},
    },
    Voice: vango.VoiceOutput("your-cartesia-voice-id"),
})
...
resp, err := client.Messages.Create(ctx, &vango.MessageRequest{
    Model: "anthropic/claude-sonnet-4",
    Messages: []vango.Message{
        {Role: "user", Content: vango.Audio(audioData, "audio/wav")},
    },
    Voice: vango.VoiceFull("your-cartesia-voice-id"),
})
...
func TestTranscribe_Cartesia(t *testing.T) {
    ...
    transcript, err := client.Audio.Transcribe(context.Background(), &vango.TranscribeRequest{
        Audio:    audio,
        Language: "en",
    })
...
func TestSynthesize_Cartesia(t *testing.T) {
    ...
    result, err := client.Audio.Synthesize(context.Background(), &vango.SynthesizeRequest{
    ...
    resp, err := client.Messages.Create(context.Background(), &vango.MessageRequest{
        ...
        Messages: []vango.Message{
            {Role: "user", Content: []vango.ContentBlock{
                vango.Audio(audio, "audio/wav"),
            }},
        },
        Voice: vango.VoiceInput(vango.WithLanguage("en")),
    })
...
func TestStream_WithVoiceOutput(t *testing.T) {
    ...
    stream, err := client.Messages.Stream(context.Background(), &vango.MessageRequest{
        Model: "anthropic/claude-sonnet-4",
        Messages: []vango.Message{
            {Role: "user", Content: vango.Text("Say hello in one sentence.")},
        },
        Voice: vango.VoiceOutput("a0e99841-438c-4a64-b679-ae501e7d6091"),
    })
    require.NoError(t, err)
    defer stream.Close()
...
func TestStreamSynthesize_Cartesia(t *testing.T) {
    ...
    stream, err := client.Audio.StreamSynthesize(context.Background(), &vango.SynthesizeRequest{
