...
func TestMessages_Stream_VoiceOutput(t *testing.T) {
	...
	stream, err := testClient.Messages.Stream(ctx, &vango.MessageRequest{
		Model: "anthropic/claude-haiku-4-5-20251001",
		Messages: []vango.Message{
			{Role: "user", Content: vango.Text("Say a greeting in one short sentence.")},
		},
		Voice: &vango.VoiceConfig{
			Output: &vango.VoiceOutputConfig{
				Voice:  "a0e99841-438c-4a64-b679-ae501e7d6091",
	...
	for event := range stream.Events() {
		...
		case vango.ContentBlockDeltaEvent:
			if _, ok := e.Delta.(vango.TextDelta); ok {
				gotTextDelta = true
			}
		}
	}
...
func TestVoice_TextWithAudioMarkers(t *testing.T) {
	...
	resp, err := testClient.Messages.Create(ctx, &vango.MessageRequest{
		Model: "anthropic/claude-haiku-4-5-20251001",
		Messages: []vango.Message{
			{Role: "user", Content: vango.Text("Count from one to five.")},
		},
		Voice: &vango.VoiceConfig{
			Output: &vango.VoiceOutputConfig{
				Voice:  "a0e99841-438c-4a64-b679-ae501e7d6091",
...
func TestVoice_SpeedVariation(t *testing.T) {
	...
	for _, speed := range speeds {
		resp, err := testClient.Messages.Create(ctx, &vango.MessageRequest{
			Model: "anthropic/claude-haiku-4-5-20251001",
			Messages: []vango.Message{
				{Role: "user", Content: vango.Text("Say hello world.")},
			},
			Voice: &vango.VoiceConfig{
				Output: &vango.VoiceOutputConfig{
					Voice:  "a0e99841-438c-4a64-b679-ae501e7d6091",
