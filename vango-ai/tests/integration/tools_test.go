...
func TestMessages_Create_ToolResult(t *testing.T) {
	...
	resp1, err := testClient.Messages.Create(ctx, &vango.MessageRequest{
		...
		ToolChoice: vango.ToolChoiceTool("get_weather"),
		MaxTokens:  200,
	})
	if err != nil {
	...
	if resp1.StopReason != vango.StopReasonToolUse {
	...
	if resp2.StopReason != vango.StopReasonEndTurn {
...
func TestMessages_Create_ToolChoice_Auto(t *testing.T) {
	...
	resp, err := testClient.Messages.Create(ctx, &vango.MessageRequest{
		Model: "anthropic/claude-haiku-4-5-20251001",
		Messages: []vango.Message{
			{Role: "user", Content: vango.Text("What is 2+2?")},
		},
		Tools: []vango.Tool{
			{
				Type:        "function",
		...
		ToolChoice: vango.ToolChoiceAuto(),
		MaxTokens:  100,
	})
	...
	if resp.StopReason != vango.StopReasonEndTurn && resp.StopReason != vango.StopReasonToolUse {
...
func TestMessages_Create_ToolChoice_None(t *testing.T) {
	...
	resp, err := testClient.Messages.Create(ctx, &vango.MessageRequest{
		Model: "anthropic/claude-haiku-4-5-20251001",
		Messages: []vango.Message{
			{Role: "user", Content: vango.Text("What's the weather? Use a tool if needed.")},
		},
		Tools: []vango.Tool{
			{
				Type:        "function",
		...
		ToolChoice: vango.ToolChoiceNone(),
		MaxTokens:  100,
	})
...
func TestFuncAsTool_SchemaGeneration(t *testing.T) {
	...
	tool, handler := vango.FuncAsTool("test_tool", "A test tool",
...
func TestMakeTool_Integration(t *testing.T) {
	tool := vango.MakeTool("greet", "Greet a person",
