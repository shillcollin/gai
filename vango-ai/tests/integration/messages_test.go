...
func TestMessages_Create_SimpleText(t *testing.T) {
	...
	resp, err := testClient.Messages.Create(ctx, &vango.MessageRequest{
	...
	if resp.StopReason != vango.StopReasonEndTurn {
...
func TestMessages_Create_MultiTurn(t *testing.T) {
	...
	// First turn
	resp1, err := testClient.Messages.Create(ctx, &vango.MessageRequest{
		Model: "anthropic/claude-haiku-4-5-20251001",
		Messages: []vango.Message{
	...
	resp2, err := testClient.Messages.Create(ctx, &vango.MessageRequest{
		Model: "anthropic/claude-haiku-4-5-20251001",
		Messages: []vango.Message{
			{Role: "user", Content: vango.Text("My name is Alice. Remember this.")},
	...
	resp, err := testClient.Messages.Create(ctx, &vango.MessageRequest{
		...
		Messages: []vango.Message{
			{Role: "user", Content: []vango.ContentBlock{
				vango.Text("What is shown in this image? Be very brief, just name the object."),
				vango.ImageURL("https://upload.wikimedia.org/wikipedia/commons/thumb/3/3a/Cat03.jpg/120px-Cat03.jpg"),
			}},
		},
		MaxTokens: 50,
	})
...
func TestMessages_Create_MaxTokens(t *testing.T) {
	...
	if resp.StopReason != vango.StopReasonMaxTokens {
...
func TestMessages_Create_StopSequences(t *testing.T) {
	...
	if resp.StopReason != vango.StopReasonStopSequence {
...
func TestMessages_Extract(t *testing.T) {
	...
	var person Person
	_, err := testClient.Messages.Extract(ctx, &vango.MessageRequest{
		Model: "anthropic/claude-haiku-4-5-20251001",
		Messages: []vango.Message{
